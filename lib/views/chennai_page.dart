...
class ChennaiPage extends StatefulWidget {
  const ChennaiPage({super.key});

  @override
  State<ChennaiPage> createState() => _ChennaiPageState();
}

class _ChennaiPageState extends State<ChennaiPage> {
  final controller = AirQualityController();

  final List<String> stationNames = ['Velachery', 'Alandur', 'Arumbakkam'];
  ...
  Widget build(BuildContext context) {
    final screenHeight = MediaQuery
        ...
        .size
        .height;
    final cardHeight = (screenHeight - kToolbarHeight - 80) /
        3; // fit 3 cards nicely

    return Scaffold(
      appBar: AppBar(
        title: const Text("Chennai - Air Quality"),
  ...
  String formatDateTime(String? dateTimeStr) {
    ...
    try {
      ...
      return "${parsed.hour.toString().padLeft(2, '0')}:${parsed.minute
          .toString().padLeft(2, '0')}  ${parsed.day}/${parsed.month}/${parsed
          .year}";
    } catch (e) {
      return "Invalid";
    }
